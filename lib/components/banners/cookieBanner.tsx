...
export default function CookieBanner() {
  ...
  return (
    <section
      className={
        "fixed max-w-2xl p-4 mx-10 border md:gap-x-4 right-0 bottom-10 bg-gray-900 md:items-center border-gray-700 rounded-2xl" +
        (cookieConsent !== null ? " hidden" : " md:flex")
      }
    >
      <div className="flex items-center gap-x-4">
        <span className="self-start mt-2 inline-flex text-accent rounded-lg shrink-0">
          <svg
      ...
      <div className="flex flex-col items-center mt-6 gap-2 shrink-0 lg:mt-0">
        <button
          className="text-sm font-medium bg-gray-700 rounded-lg hover:bg-gray-800 text-white leading-none px-4 py-2.5 duration-300 transition-colors focus:outline-none"
          onClick={() => setCookieConsent(true)}
        >
          Allow Cookies
        </button>
        <button
          className="text-xs underline transition-colors duration-300 md:w-auto text-white hover:text-gray-400 focus:outline-none"
